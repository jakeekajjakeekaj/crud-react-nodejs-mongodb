...
// Una vez finalizado, ahora debemos indicar qué rutas serán privadas, para este caso lo que es el HomePage, LoginPage y RegisterPage serán públicas, mientras que TasksPage, TaskFormPage y ProfilePage deberán ser solo para usuarios logeados, osea que deben ser privadas.

// Una manera de realizar la validación de rutas protegidas es dentro del componente de la página, por ejemplo dentro de TaskPage indicarle que antes de renderizar, se debe comprobar si existe una cookie para continuar y de lo contrario redireccionar a otra página, sin embargo esto no sería lo más ideal ya que debemos pegar ese fragmento de código en todos los componentes de las páginas, lo que haría repetirse demasiado el código, es por esto que mejor se creará un componente que se encargue de esto para así pasar por el mismo antes de renderizar el componente de la página, por lo tanto dentro de App.jsx se colocarán a todas las rutas privadas dentro de un Route element, dentro de src del client se creará un componente llamado ProtectedRoute.jsx y escribiremos código dentro y así al estar creado este componente, sumado a que en App.jsx todos los componentes privados se encuentran contenidos dentro de este componente ProtectedRoute, ya podemos poner a prueba las redirecciones y verificar que se redireccione antes del reenderizado.

// Ahora para que estas rutas que funcionen como se espera una vez que nos hemos logeado, aparte de cambiar algunas cositas del signin ubicado dentro del AuthContext para que la información quede guardada, pero a parte de eso, nosotros al cargar la página el isAuthenticated se setea en false y nos redirecciona (cosa que no debe pasar), por lo que dentro del AuthContext crearemos unas lineas para que esto no pase, así mismo necesitaremos leer la cookie del lado front por JS, por lo que instalaremos en el client "npm i js-cookie"
// Una vez instalado, en el AuthContext importamos Cookies y escribimos código dentro del useEffect, ahora para evitar errores, crearemos un archivo dentro de api llamado axios.js y escribimos código; dentro del archivo configuraremos axios.
// Ahora dentro de auth.js importamos este archivo creado llamado axios.js, ahora nos tendrá un error de CORS, para solucionar esto, nos dirigimos a la app.js de nuestro backend y escribimos en el cors, abajo del origin, colocamos credentials: true, de esta manera indicamos que se admitirá el uso de las creenciales y listo, ya se queda guardado el token, ahora lo que se tiene que hacer es en AuthContext.jsx indicar que de existir las cookies, se pidan los datos al backend, por lo que dentro de api/auth.js se creará otra ruta /auth/verify
...
// ----------- ACTUALIZAR TAREAS (EDIT) -----------

// Nos dirigimos a nuestra TaskCard y dentro del botón de Edit, lo reemplazamos por un Link y direccionamos para la dirección del PUT, hasta ahorita noe stará actualizando nada, ya que como tal nos dirige a la página de creación de Tasks, pero podemos darnos cuenta del efecto en la dirección URL, ya que al crear una task la dirección URL nos muestra únicamente el de createtask, sin embargo al momento de editar, ahora sí nos muestra todo el número identificador de la task correspondiente
// Ahora nos dirigiremos al TaskFormPage y dentro del mismo importaremos un UseEffect y un useParams, el useParams es de react-router-dom y básicamente es usado para convertir en objeto a la URL dinpamica de nuestro buscador, ahora SE HA VISTO QUE SI HAY ALGÚN ERROR A NIVEL BACKEND, todo nuestro backend se caerá, para evitar esto, en la sección del backend, dentro de controllers y tasks.controller, empezamos a colocar un try catch a todas las funciones contenidas en el mismo, esto con el motivo de evitar que nuestro backend entero se caiga en caso de existir algún error
// Ahora modificamos los datos en TaskFormPage.jsx y en TasksContext.jsx, sin embargo, hasta ahorita solo se tienen los datos ya reflejados al momento de querer editar nuestras tareas, sin embargo todavía no se actualizarían estos cambios.
// Para esto primero nos fijamos en el onSubmit, una vez esto, metemos código para poder actualizar las tasks tanto en el TaskFormPage.jsx, en el TasksContext.jsx y aparte modificaremos la api que teniamos en tasks.js, esto porque nosotros en el parámetro de entrada teniamos solo la task, cuando teniamos que tener un id y la task. Una vez finalizado ahora sí ya podemos editar la información y esta se almacenará.

// ------------------- RETOQUES -------------------

// Ahora ya solo se procederá a mejorar lo que es la aplicación, para que esta se vea mejor en todos los sentidos
// Nosotros al mejorar desde el TaskFormPage.jsx nuestra manera de agregar las tareas por ejemplo, usamos un input date, el problema de esto es que al querer guardar los valores nuestro back no los aceptará, esto porque no acepta este formato de fecha, por lo que para resolver estonos dirigiremos a nuestro back, en controllers y dentro del tasks.controller.js verificamos que efectivamnte estamos esperando la Date, pero no solo eso, sino que en el models también dentro de nuestro task.model.js estamos indicanod el tipo de dato que esperamos, por lo que para poder corregir este error, debemos instalar para el lado front una biblioteca: "npm i dayjs", esta sirve para cambiar en distintos formatos a la fecha, después lo que se debe hacer es dirigirnos al lado front y dentro del TaskFormPage.jsx y agregamos el código correspondiente para trabajar con la librería, así mismo, la fecha no nos está formateando de forma correcta, es por esto que nos dirigiremos al TaskCard.jsx e importamos el utc, dayjs y usamos el extend de igual manera para poder trabajar con la librería pero ahora desde este componente.
// Una vez finalizado el proceso, podemos ver que ya hemos formateado también la manera en que la fecha se muestra dentro de nuestras Tasks Cards
// Ahora nadamás mejoramos un poco más el estilo para los botones de delete y edit dentro del componente de TaskCard.jsx
// Hemos mejorado el estilo del button en Login y Register, así como los h1 de ambos componentes
// Ahora dentro de TaskFormPage corregimos que se muestre la feha de forma correcta al momento de editar algo, es decir que antes no aparecía la fecha inicial, y ahora ya se consiguió que esta aparezca